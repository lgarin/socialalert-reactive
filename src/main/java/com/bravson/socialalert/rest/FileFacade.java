...
public class FileFacade {
	...
	private ResponseEntity<FileSystemResource> createStreamResponse(FileResponse fileResponse) {
		...
		if (fileResponse.isTemporary()) {
			headers.setCacheControl(CacheControl.maxAge(mediaCacheMaxAge, TimeUnit.SECONDS));
		}
	...
	public ResponseEntity<FileSystemResource> download(
			...
			@NotEmpty @PathVariable("fileUri") String fileUri) throws IOException {
		return createStreamResponse(fileReadService.download(fileUri).orElseThrow(MediaFileNotFoundException::new));
	}
	...
	public ResponseEntity<FileSystemResource> preview(
			@Parameter(description="The relative file uri.", required=true) @NotEmpty @PathVariable("fileUri") String fileUri) throws IOException {
		return createStreamResponse(fileReadService.preview(fileUri).orElseThrow(MediaFileNotFoundException::new));
	}
	...
	public ResponseEntity<FileSystemResource> thumbnail(
			@Parameter(description="The relative file uri.", required=true) @NotEmpty @PathVariable("fileUri") String fileUri) throws IOException {
		return createStreamResponse(fileReadService.thumbnail(fileUri).orElseThrow(MediaFileNotFoundException::new));
	}
	...
	public ResponseEntity<?> uploadPicture(
		...
		return createUploadResponse(fileUploadService.uploadMedia(createUploadParameter(inputFile, contentType), userAccess));
	}
